...
| 场景 | 构图策略 | 主体占比 | 核心目标 |
|-----|---------|---------|---------|
| 宣传海报 | 详见banana-app-prompt_generator业务层配置（8种构图可选：叙事留白式/对角线动态式/中心稳重式/S曲线探索式/宏大仰视式/低角冲击式/中心爆发式/天空主导式） | 多档可选：15-20%、45-60%、75-90% | 视觉冲击/情感共鸣/叙事感/传达氛围 |
...
| 缺少光线描述 | 图标→柔和内发光；海报→戏剧性逆光；功能图→均匀工作室光 |
| 构图不完整 | 图标→居中圆形构图；海报→对角线三分法；功能图→等距视角 |
| 风格不一致 | 替换为对应分类的渲染风格关键词 |
| 场景构图策略错误 | 图标→「主体居中充满，干净背景」；海报→「电影级广角，渺小主体衬宏伟背景」；功能图→「等距俯视，元素清晰排列」 |
| 品牌色缺失 | 显式写入具体色彩描述 |
