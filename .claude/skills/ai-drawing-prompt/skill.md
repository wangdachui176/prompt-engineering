...
- **负面提示词必须输出**，每个 prompt 必须包含负面提示词部分

---

## 强制负面词约束（所有场景必须包含）

**每个生成的 prompt 的负面提示词部分，必须强制包含以下内容（中文）：**

```
真实人物，真人面孔，写实人像，儿童，婴儿，幼儿，幼稚风格，低龄画风，
暴力，血腥，伤害，武器，黑暗主题，负面情绪，悲伤，愤怒，恐惧，抑郁，
画面模糊，失焦，低分辨率，构图不完整，主体被裁切，画面残缺
```

- **禁止真人**：真实人物，真人面孔，写实人像
- **禁止低龄/幼稚**：儿童，婴儿，幼儿，幼稚风格，低龄画风
- **禁止暴力血腥**：暴力，血腥，伤害，武器
- **禁止负面情绪**：负面情绪，悲伤，愤怒，恐惧，抑郁，哭泣
- **禁止模糊不清**：画面模糊，失焦，低分辨率，噪点
- **禁止构图不完整**：构图不完整，主体被裁切，画面残缺
...
**堆叠式常用词库：**
- 动态感：`狂风呼啸`、`空间扭曲`、`气流汹涌`、`风卷残叶`、`速度线残影`、`动态模糊`
- 光影：`过曝+双重曝光`、`高对比+强烈对比色碰撞`、`辉光+耀光+雾感`、`曜变效果`、`珠光反光质感`
- 质感：`强烈胶片颗粒与高噪点`、`弥散晕染暗部层次`、`HDR写实质感`、`边缘虚化`
- 氛围：`梦核美学`、`意识流+抽象表达`、`惊悚元素`、`宿命感`、`压迫感与威胁感`
- 镜头：`超强透视+超广角强透视`、`全景航拍+电影镜头`、`柔和测光+微光渲染`

### 即梦负面提示词规范

即梦支持负面提示词，建议根据场景类型选用：

**通用基础（每次都加）：**
`画面模糊，低分辨率，水印，文字，多余肢体，手指畸形，比例失调，丑陋`

**人物场景追加：**
`多余的手，六根手指，面部扭曲，眼睛不对称，皮肤质感差`

**建筑/场景追加：**
`透视错误，结构扭曲，漂浮物体，不合理阴影`

**风格保护（防止风格污染）：**
- 要写实时加：`卡通，动漫，插画，扁平化`
- 要动漫时加：`写实摄影，照片质感，过度真实`
...
负面提示词：画面模糊，低分辨率，水印，多余肢体，手指畸形，卡通，扁平化
```
...
bokeh background, highly detailed, 4K resolution

Negative: blur, distorted face, extra fingers, low quality, watermark, photorealistic, photo
```
...
Negative: blurry, artifacts, oversaturated, distorted perspective, cartoon, anime, daytime
```
